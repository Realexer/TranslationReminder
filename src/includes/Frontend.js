var Frontend = function()
{
   ...
   this.refreshCallback = function(words)
   {
      ...
      {
	 this.checkPageForAttentionWords(textNodes[i], textNodesValues[i]);
      }
   ...
   this.translateWithBing = function()
   {
	 ...
	 s.src = "http://api.microsofttranslator.com/V2/Ajax.svc/Translate?oncomplete=bingcallback&appId=8E54095330F0B7E7CB73527A50437E6110A64730&to=" + "ru" + "&text=" + currentSelection;
   ...
   this.showHint = function(event)
   {
	 ...
	 if (child.className === hintClassName) 
	 {
      ...
      {
	 this.deleteHint(event);
      }, true);

      document.getElementById('deleteWordSpan').onclick = function()
      {
	 this.deleteWord(curTarget.firstChild.nodeValue);
